...
class MediaSeg(SegmentBase):
    ...
    def build(cls, file: str):
        if file.startswith(("http://", "https://", "base64://")):
        ...
        if file.startswith("file://"):
            ...
            if os.path.isfile(file):
                if os.path.isabs(file):
                    try:
                        return cls(file=pathlib.Path(file).absolute().as_uri())
                    except Exception:
                        res = cls()
                    ...
                    try:
                        return cls(file=pathlib.Path(os.path.abspath(file)).absolute().as_uri())
                    except Exception:
                        res = cls()
