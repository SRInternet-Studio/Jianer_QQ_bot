    ...
    def milky_outgoing_seg(self) -> dict:
        def normalize_uri(uri: str) -> str:
            if not uri:
                return ""
            u = str(uri).strip()
            if u.startswith(("http://", "https://", "base64://")):
                return u
            if u.startswith("file://"):
                raw = u[len("file://"):].lstrip("/")
                raw = raw.replace("/", os.sep).replace("\\", os.sep)
                try:
                    return pathlib.Path(raw).absolute().as_uri()
                except Exception:
                    return u
            try:
                return pathlib.Path(u).absolute().as_uri()
            except Exception:
                return u
        ...
        if ma("text"):
            ...
            return builder.image(normalize_uri(data["data"]["file"]), data["data"].get("summary", "[Image]")).build()[0]
        elif ma("at"):
            ...
            return builder.record(normalize_uri(data["data"]["file"])).build()[0]
        elif ma("video"):
            return builder.video(normalize_uri(data["data"]["file"])).build()[0]
        else:
