...
class Event(ABC):
    def __init__(self, data: dict):
        ...
        owner_set = {str(i) for i in config.owner}
        feishu_cfg = config.others.get("feishu", {}) if isinstance(config.others, dict) else {}
        owner_set.update(str(i) for i in feishu_cfg.get("allowed_open_ids", []))
        self.is_owner = Integer.convert_from(self.user_id) in config.owner or str(self.user_id) in owner_set

        black_list = {str(i) for i in config.black_list}
        silents = {str(i) for i in config.silents}
        self.blocked = str(self.user_id) in black_list or str(self.group_id) in black_list
        self.is_silent = str(self.user_id) in silents or str(self.group_id) in silents or "0" in silents or 0 in config.silents
