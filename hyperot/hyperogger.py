...
nf_icons = NerdICONs(getattr(config, "log_use_nf", False))
...
class Logger:
    ...
    def create(cls, key: str, level: str):
        ...
        c.debug(key)
        c.debug(str(c))
        c.debug(str(cls.running_loggers))
        return c

    @classmethod
    def fetch(cls, key: str):
        logger = cls.running_loggers.get(key)
        if logger:
            logger.debug(str(cls.running_loggers))
        return logger
