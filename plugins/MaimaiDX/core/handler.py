...
class LxnsBindingTokenStorageError(Exception):
    """The OAuth code was exchanged, but its token could not be persisted."""


class LxnsBindingPostPersistError(Exception):
    """The OAuth token is persisted, but optional player metadata failed."""
...
async def bind_lxns(user: User, code: str) -> str:
    ...
    try:
        token_update = await update_user(user.qqid, token=token)
    except Exception as error:
        raise LxnsBindingTokenStorageError from error
    if token_update is None:
        raise LxnsBindingTokenStorageError

    try:
        friend_code = await get_friend_code(user.qqid, token)
        friend_code_update = await update_user(user.qqid, friend_code=friend_code)
        if friend_code_update is None:
            raise RuntimeError("friend code update returned no user")
    except Exception as error:
        raise LxnsBindingPostPersistError from error
    return "授权完成。"
